...
Camera::Camera(float width, float height, float nearPlane, float farPlane)
    : Entity(0.0f, 0.0f, 0.0f),
      ...
      viewMatrix(glm::lookAt(transform.Position, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f))) 
      {}

void Camera::updateViewMatrix() {
    viewMatrix = glm::lookAt(transform.Position, transform.Position + getForward(), getUp());
}
